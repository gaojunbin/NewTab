...
function SortableApp({ app, settings, onEdit, onDelete }: SortableAppProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    <div ref={setNodeRef} style={style} className="relative">
      ...
      >
        ...
        <div
          {...attributes}
          {...listeners}
          className="absolute top-1 left-1 p-1 cursor-grab active:cursor-grabbing rounded opacity-50 hover:opacity-100 transition-opacity"
...
function AppGrid({ appGroups }: AppGridProps) {
  const { deleteApp, deleteAppGroup, reorderApps } = useAppStore();
  ...
  const handleDragEnd = (event: DragEndEvent, groupId: string, apps: App[]) => {
    const { active, over } = event;
    if (over && active.id !== over.id) {
      const oldIndex = apps.findIndex((app) => app.id === active.id);
      const newIndex = apps.findIndex((app) => app.id === over.id);
      reorderApps(groupId, oldIndex, newIndex);
    }
  };
        ...
        >
          ...
          {isEditMode ? (
            <DndContext
              ...
              onDragEnd={(event) => handleDragEnd(event, group.id, group.apps)}
            >
              <SortableContext items={group.apps.map(app => app.id)} strategy={rectSortingStrategy}>
                <div className="grid grid-cols-3 xs:grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-3 sm:gap-4">
                  {group.apps.map((app) => (
                    <SortableApp
                      ...
                      onDelete={() => { if (confirm('Delete?')) deleteApp(group.id, app.id); }}
                    />
                  ))}
                  {/* Add App Button */}
                  <motion.button
